...
impl DatHeader {
    ...
    pub fn read_from<R: Read>(r: &mut R) -> io::Result<Self> {
        ...
        if &magic != DAT_MAGIC {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "not a nudb data file"));
        }
        let mut buf2 = [0u8; 2];
        let mut buf8 = [0u8; 8];
        r.read_exact(&mut buf2)?; let version = u16::from_le_bytes(buf2);
        r.read_exact(&mut buf8)?; let uid = u64::from_le_bytes(buf8);
        r.read_exact(&mut buf8)?; let appnum = u64::from_le_bytes(buf8);
        r.read_exact(&mut buf2)?; let key_size = u16::from_le_bytes(buf2);
        let mut reserved = [0u8; 64];
        r.read_exact(&mut reserved)?;
        Ok(Self { version, uid, appnum, key_size })
    }
}
...
impl KeyHeader {
    ...
    pub fn read_from<R: Read>(r: &mut R) -> io::Result<Self> {
        ...
        r.read_exact(&mut magic)?;
        if &magic != KEY_MAGIC {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "not a nudb key file"));
        }
        let mut b2 = [0u8; 2];
        let mut b8 = [0u8; 8];
        r.read_exact(&mut b2)?; let version = u16::from_le_bytes(b2);
        r.read_exact(&mut b8)?; let uid = u64::from_le_bytes(b8);
        r.read_exact(&mut b8)?; let appnum = u64::from_le_bytes(b8);
        r.read_exact(&mut b2)?; let key_size = u16::from_le_bytes(b2);
        r.read_exact(&mut b8)?; let salt = u64::from_le_bytes(b8);
        r.read_exact(&mut b8)?; let pepper = u64::from_le_bytes(b8);
        r.read_exact(&mut b2)?; let block_size = u16::from_le_bytes(b2);
        r.read_exact(&mut b2)?; let load_factor = u16::from_le_bytes(b2);
        r.read_exact(&mut b8)?; let bucket_count = u64::from_le_bytes(b8);
        r.read_exact(&mut b8)?; let key_count = u64::from_le_bytes(b8);
        ...
        Ok(Self { version, uid, appnum, key_size, salt, pepper, block_size, load_factor, bucket_count, key_count })
    }
...
impl Bucket {
    pub fn new() -> Self {
        Self { count: 0, spill: 0, entries: Vec::new() }
    }
    ...
    pub fn from_bytes(data: &[u8]) -> Self {
        ...
        for i in 0..count as usize {
            ...
            if base + ENTRY_SIZE > data.len() { break; }
    ...
    pub fn to_bytes(&self, block_size: usize) -> Vec<u8> {
        ...
        for (i, e) in self.entries.iter().enumerate() {
            let base = BUCKET_HEADER_SIZE + i * ENTRY_SIZE;
            if base + ENTRY_SIZE > block_size { break; }
    ...
    pub fn insert(&mut self, entry: BucketEntry) {
        let pos = self.entries.iter().position(|e| e.hash >= entry.hash)
